...
export default function WorkspacePlanner({ todos, onTaskUpdated, onError }: Props) {
  ...
  const unscheduled = picked.flatMap((todo) => todo.todo_items.filter((item) => item.scheduled_start?.slice(0, 10) !== today).map((item) => ({ todo, item })))
  ...
  async function saveItem(task: Todo, itemId: number, changes: Partial<TodoItemInput>) {
    try {
      const todo_items = task.todo_items.map((item) => ({
        ...
        scheduled_start: item.scheduled_start, scheduled_duration_minutes: item.scheduled_duration_minutes,
  ...
  function place(event: DragEvent<HTMLDivElement>, task: Todo) {
    ...
    const duration = (item.scheduled_duration_minutes ?? item.estimated_duration_minutes) || 60
    void saveItem(task, item.id, { scheduled_start: dateTime(today, Math.min(minutes, DAY_MINUTES - duration)), scheduled_duration_minutes: duration })
  }
  ...
  function resize(event: ReactPointerEvent<HTMLSpanElement>, task: Todo, item: TodoItem, edge: 'start' | 'end') {
    ...
    if (!track || !item.scheduled_start) return
    const originalStart = minutesOfDay(item.scheduled_start)
    const originalDuration = (item.scheduled_duration_minutes ?? item.estimated_duration_minutes) || 60
    ...
    const stop = (pointer: PointerEvent) => {
      ...
      void saveItem(task, item.id, { scheduled_start: dateTime(today, nextStart), scheduled_duration_minutes: nextDuration })
    }
    window.addEventListener('pointermove', move); window.addEventListener('pointerup', stop)
  }

  return <section className="workspace-planner">
    <header><div><p className="eyebrow">Today</p><h2>{new Intl.DateTimeFormat('en', { dateStyle: 'full' }).format(new Date())}</h2></div><small>Drag todo items onto a picked task’s timeline · resize from either edge</small></header>
      ...
      <div className="workspace-planner-grid">
        ...
        {picked.length === 0 ? <div className="workspace-planner-empty">Pick a task from the left sidebar to add it to today’s workspace.</div> : picked.map((task) => <div className="workspace-task-row" key={task.id}>
          ...
          <div className="workspace-time-track" onDragOver={(event) => event.preventDefault()} onDrop={(event) => place(event, task)}>
            {task.todo_items.filter((item) => item.scheduled_start?.slice(0, 10) === today).map((item) => {
              const start = minutesOfDay(item.scheduled_start!)
              const duration = (item.scheduled_duration_minutes ?? item.estimated_duration_minutes) || 60
              return <div className="workspace-todo-block" draggable onDragStart={(event) => { event.dataTransfer.setData('text/todo-item-id', String(item.id)); event.dataTransfer.effectAllowed = 'move' }} style={{ left: `${start / DAY_MINUTES * 100}%`, width: `${duration / DAY_MINUTES * 100}%` }} title={`$${item.id} ${item.name} · ${formatMinutes(duration)}`} key={item.id}>
    ...
    {unscheduled.length > 0 && <section className="workspace-unscheduled"><h3>Unscheduled todo items</h3><div>{unscheduled.map(({ todo, item }) => <div draggable onDragStart={(event) => { event.dataTransfer.setData('text/todo-item-id', String(item.id)); event.dataTransfer.effectAllowed = 'move' }} key={item.id}><b>${item.id}</b><span>{item.name}</span><small>#{todo.id} · {formatMinutes(item.estimated_duration_minutes)}</small></div>)}</div></section>}
  </section>
}
